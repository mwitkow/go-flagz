...
func (s *endpointTestSuite) SetupTest() {
	s.flagSet = pflag.NewFlagSet("foobar", pflag.ContinueOnError)
...
func (s *endpointTestSuite) TestReturnsAll() {
	...
	list := s.processFlagzJsonResponse(req)
...
func (s *endpointTestSuite) TestCorrectlyRepresentsResources() {
	...
	assert.Equal(s.T(),
		...
		findFlagInJsonList("some_static_float", list),
	...
	assert.Equal(s.T(),
		...
		findFlagInJsonList("some_dyn_stringslice", list),
...
func (s *endpointTestSuite) processFlagzJsonResponse(req *http.Request) *listJson {
	resp := httptest.NewRecorder()
	s.endpoint.ListFlags(resp, req)
	require.Equal(s.T(), http.StatusOK, resp.Code, "flagz list request must return 200 OK")
...
func (s *endpointTestSuite) assertListContainsOnly(flagList []string, list *listJson) {
	existing := []string{}
...
func findFlagInJsonList(flagName string, list *listJson) *flagJson {
	for _, f := range list.Flags {
		if f.Name == flagName {
			return f
		}
